...
# 简要分析函数get_kitti_image_info，它的输出image_info是一个字典型变量，包含'image_idx'，pointcloud_num_features，velodyne_path，img_path，img_shape，annos。
...
'''在annos中，truncated和occluded用于衡量该3d目标被遮挡的程度，用于给该3d目标被检测到的难易程度打分.
   bbox和dimensions都表示3d目标的长宽高，它们的区别见代码中的注释。location表示3d框中心点的位置。
   rotation_y表示3d框中Yaw角度。score表示目标类别置信度。num_objects表示当前点云中有多少3d目标。
   name指3d目标类别。index和group_ids指该3d目标的索引和类索引。'''
